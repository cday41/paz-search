    ...
    public class AnimalManager : System.Collections.ArrayList
    {
       ...
       private Dictionary<IPoint,MapValue> mMapValues;
        ...
        public void adjustNewSocialMap(Map newSocialMap)
        {
            try
            {
                Resident[] rs = this.getResidents();
                string fieldName;
                fw.writeLine("inside adjustNewSocialMap we are going to loop through " + rs.Length.ToString() + " residents");
                foreach (Resident r in rs)
                {
                    fw.writeLine("my type of animal is a " + r.GetType());
                    fw.writeLine("my animals location is " + r.getLocation_XY());

                    fw.writeLine(r.OriginalID + " needs " + r.HomeRangeArea.ToString() + " and the new map has " + newSocialMap.getAllAvailableArea(System.Convert.ToInt32(r.OriginalID), r.Sex).ToString());
                    if (r.HomeRangeArea > newSocialMap.getAllAvailableArea(System.Convert.ToInt32(r.OriginalID), r.Sex))
                    {
                        fw.writeLine("not enough so kill the bugger off");

                        r.TextFileWriter.addLine("Not enough suitable habitat after map switch.");
                        ...
                        fw.writeLine("new animal type is " + r.GetType());
                        ...
                        newSocialMap.resetFields(fieldName, r.OriginalID.ToString(), "none");
        ...
        public bool makeResidents(InitialAnimalAttributes[] inResAttributes, int currYear)
        {
            bool success = false;
            Resident r;
            ...
            try
            {
                for (int i = 0; i < inResAttributes.Length; i++)
                {
                    ...
                    r.OriginalID = inResAttributes[i].OrginalID;
                    r.buildFileNamePrefix(currYear.ToString());
                    r.TextFileWriter = new TextFileWriter(this.AnimalAttributes.OutPutDir, r.FileNamePrefix);
       ...
       public void setInitialValues()
       {
        ...
        private Resident[] getResidents()
        {
            ...
            List<Resident> tempRes = new List<Resident>();
        ...
        private void setMaleHomeRange()
        {
            ...
            for (int i = 0; i < numAnimals; i++)
            {
                ...
                if (!tmpAnimal.IsDead)
                {
                    ...
                    if (String.Compare(tmpAnimal.Sex, "male", true) == 0)
                    {
        ...
        private void setFemaleHomeRange()
        {
            ...
            for (int i = 0; i < numAnimals; i++)
            {
                ...
                if (!tmpAnimal.IsDead)
                {
                    //fw.writeLine("inside the loop checking to see if it is a male");
                    //fw.writeLine("the animal id is " + tmpAnimal.IdNum.ToString());
                    if (String.Compare(tmpAnimal.Sex, "female", true) == 0)
