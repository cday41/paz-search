...
namespace SEARCH
{
   ...
   public class AnimalAtributes
   {
      ...
      {
          String output = "";
          output += "AnimalAttributes-mInitialEnergy:" + this.mInitialEnergy.ToString() + ", ";
          output += "AnimalAttributes-mMaxEnergy:" + this.mMaxEnergy.ToString() + ", ";
          output += "AnimalAttributes-mMinEnergy_Survive:" + this.mMinEnergy_Survive.ToString() + ", ";
          output += "AnimalAttributes-mWakeUpTime:" + this.mWakeUpTime.ToString() + ", ";
          output += "AnimalAttributes-mPerceptionDistance:" + this.mPerceptionDistance.ToString() + ", ";
          output += "AnimalAttributes-mForageSearchTrigger:" + this.mForageSearchTrigger.ToString() + ", ";
          output += "AnimalAttributes-mSafeRiskyTrigger:" + this.mSafeRiskyTrigger.ToString() + ", ";
          output += "AnimalAttributes-mSafeProbGetKilledMod:" + this.mSafeProbGetKilledMod.ToString() + ", ";
          output += "AnimalAttributes-mSafeProbCaptureFoodMod:" + this.mSafeProbCaptureFoodMod.ToString() + ", ";
          output += "AnimalAttributes-RiskyProbCaptureFoodMod:" + this.RiskyProbCaptureFoodMod.ToString() + ", ";
          output += "AnimalAttributes-mRiskyProbGetKilledMod:" + this.mRiskyProbGetKilledMod.ToString() + "\n";
          output += "AnimalAttribtues-ActivityDurations-Start:" + this.myActivityDuration.Length.ToString() + "\n";
          for (int i = 0; i < this.myActivityDuration.Length; i++) //Need to get every duration in myActivityDuration
          {
              output += String.Format("Duration:{0},", i);
              output += "MeanAmt:" + this.myActivityDuration[i].MeanAmt.ToString() + ",";
              output += "StandardDeviation:" + this.myActivityDuration[i].StandardDeviation.ToString() + ",";
              output += "Type:" + this.myActivityDuration[i].Type.ToString() + "\n";
          }
          return output;
      }
      ...
      public bool loadBackup(string line, int i)
      {
          ...
          try
          {
              if (i == 0)
              {
                  ...
                  this.mInitialEnergy = Convert.ToDouble(pair[1].Trim());
                  pair = words[1].Split(':');
                  this.mMaxEnergy = Convert.ToDouble(pair[1].Trim());
                  pair = words[2].Split(':');
                  this.mMinEnergy_Survive = Convert.ToDouble(pair[1].Trim());
                  pair = words[3].Split(':');
                  this.mWakeUpTime = Convert.ToDouble(pair[1].Trim());
                  pair = words[4].Split(':');
                  this.mPerceptionDistance = Convert.ToDouble(pair[1].Trim());
                  pair = words[5].Split(':');
                  this.mForageSearchTrigger = Convert.ToDouble(pair[1].Trim());
                  pair = words[6].Split(':');
                  this.mSafeRiskyTrigger = Convert.ToDouble(pair[1].Trim());
                  pair = words[7].Split(':');
                  this.mSafeProbGetKilledMod = Convert.ToDouble(pair[1].Trim());
                  pair = words[8].Split(':');
                  this.mSafeProbCaptureFoodMod = Convert.ToDouble(pair[1].Trim());
                  pair = words[9].Split(':');
                  this.mRiskyProbCaptureFoodMod = Convert.ToDouble(pair[1].Trim());
                  pair = words[10].Split(':');
                  this.mRiskyProbGetKilledMod = Convert.ToDouble(pair[1].Trim());
              }
              else
              {
                  string[] words = line.Split(',');
                  string[] pair = words[0].Split(':');
                  int count = Convert.ToInt32(pair[1].Trim());
                  pair = words[1].Split(':');
                  this.myActivityDuration[count].MeanAmt = Convert.ToDouble(pair[1].Trim());
                  pair = words[2].Split(':');
                  this.myActivityDuration[count].StandardDeviation = Convert.ToDouble(pair[1].Trim());
                  pair = words[3].Split(':');
                  this.myActivityDuration[count].Type = pair[1].Trim();
              }
          }
          catch
          {
