...
namespace PAZ_Dispersal
{
   ...
   public class EligibleHomeSites:System.Collections.ArrayList
   {
      ...
      public EligibleHomeSite getFirstSuitableSite()
      {
         ...
         {
           if(this.Count > 0)           
               ehs = this[0] as EligibleHomeSite;
         }
         catch(System.Exception ex)
         {
#if (DEBUG)
      ...
      {
         fw.writeLine("inside getQualifiedSites");
         RandomNumbers rn = RandomNumbers.getInstance();
         double luckyNumber = rn.getUniformRandomNum();
         fw.writeLine("the lucky number is " + luckyNumber.ToString());
         ...
         foreach (EligibleHomeSite ehs in this)
         {
            ...
            {
               fw.writeLine("so we add this site to the list");
               qualifyingSites.Add(ehs);
            }
         }
         fw.writeLine("leaving with " + qualifyingSites.Count.ToString() + " sites");
