   ...
   public sealed class BestComboHomeRangeFinder : HomeRangeFinder
   {
      ...
      public override bool setHomeRangeCenter(Animal inAnimal, string inFileName)
      {
         ...
         if (this.setSuitableSites(inAnimal, inFileName))
         {
            ...
            {
               // now get the steps that are in the big enough 
                List <EligibleHomeSite> suitableSites = this.getSuitableSteps(inFileName);

               // IEnumerable<EligibleHomeSite> q = inAnimal.MySites.MySites.i(suitableSites, new SiteComparer());              
      ...
      public void setComboRank(Animal inA)
      {
         ...
         try
         {
            for (int i = 0; i < inA.MySites.SiteCount; i++)
            {
               EligibleHomeSite ehs = inA.MySites.getSite(i);
