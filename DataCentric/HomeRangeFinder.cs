...
namespace PAZ_Dispersal
{
   ...
   public class HomeRangeFinder : IHomeRangeFinder
   {
      ...
      protected void setRanges(double inDouble, EligibleHomeSites inSites)
      {
         double d = 0;
         try
         {
      ...
      private void resetRank(EligibleHomeSites inSites)
      {
         int i = 0;
         int j = 1;

         for (i = 0, j = 1; j < inSites.Count; i++, j++)
         {
            inSites.getSite(j).Rank = inSites.getSite(i).Rank + inSites.getSite(j).Rank;
         }
         //sometimes it will only be .9999999999999987 and the random number could
         //conviebly be .9999999999989 so eliminate any chance.
         inSites.getSite(inSites.Count - 1).Rank = 1.0;
